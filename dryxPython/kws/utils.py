    ...
    if len(result) == 1:
        ...
        for line in result:
            if line.startswith(matchedRowNumberLinePrefix):
                ...
                # print line.rstrip().lstrip().rstrip('"').lstrip("ID:
                # ").replace(" Separation = ", "")
                (id, separation) = line.rstrip().lstrip().rstrip('"').lstrip(
                    "ID: ").replace(" Separation = ", "").split(',')
...
# 2012-02-29 KWS Python Cone Search code - depends on new htmCircle
# Python/C++ module.
QUICK = 1
FULL = 2
COUNT = 3
...
# 2012-03-25 KWS Added django so that we can call the Django dict cursor
# if necessary.
...
def coneSearch(ra, dec, radius, tableName, htmLevel=16, queryType=QUICK, conn=None, django=False):
    ...
    if conn:
        # We have a database connection, so use it to make a call to the
        # database
        ...
        if resultSet:
            ...
            for row in resultSet:
                if tableName == 'tcs_guide_star_cat':
                    ...
                    separation = dat.getAngularSeparation(ra, dec, math.degrees(
                        row[CAT_ID_RA_DEC_COLS[tableName][0][1]]), math.degrees(row[CAT_ID_RA_DEC_COLS[tableName][0][2]]))
                else:
                    separation = dat.getAngularSeparation(
                        ra, dec, row[CAT_ID_RA_DEC_COLS[tableName][0][1]], row[CAT_ID_RA_DEC_COLS[tableName][0][2]])
...
# Code will produce a list of objects near to our stated RA and DEC pairs,
# but does not currently eliminate duplicates.
...
def bruteForceCMFConeSearch(filename, coordinatePairs, radius):
    ...
    # for all these.  But if this is scripted to look over multiple files,
    # it's useful to know this data.
    ...
    for coord in coordinatePairs:
        for row in t:
            ...
            separation = dat.getAngularSeparation(
                coord[0], coord[1], raCat, decCat)
            if separation < radius:
                # Add the returned row (there may be more than one) and the
                # object counter
    ...
    if resultsTable:
        ...
        for name in cols.names:
            # Change names of the RA/DEC columns so the DS9 catalogue reader can
            # read them
            if name == 'RA_PSF':
                name = 'RA_J2000'
            if name == 'DEC_PSF':
